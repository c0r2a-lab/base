...
namespace game {
	...
	public:
		...
		auto ActorLoop() -> void
		{
			...
			{
				pointer->client_info = decrypt->Client_Information(DMA::BaseAddress, decrypt->peb);
				pointer->client_info_base = decrypt->Client_Base(DMA::BaseAddress, decrypt->peb, pointer->client_info);
				auto ref_def_pointer = decrypt_refdef->retrieve_ref_def();
				decrypt_refdef->ref_def_nn = DMA::Read<ref_def_t>(ref_def_pointer, sizeof(ref_def_pointer));
				...
				auto Entity_Bone_Base = decrypt->bone_base(DMA::BaseAddress, decrypt->peb);
				...
				for (int i = 0; i < globals->player_count; i++)
				{
					...
					auto Bone_Pointer_Index = decrypt->bone_index(i, DMA::BaseAddress);
